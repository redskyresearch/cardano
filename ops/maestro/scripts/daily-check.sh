...
echo "Maestro @ MAINNET" >> $filepath/$outputfile
echo "Executing script as $USER, with uid $UID" >> $filepath/$outputfile
echo "###########################" >> $filepath/$outputfile
...
export CARDANO_NODE_SOCKET_PATH=/home/ec2-user/.local/bin/relay/db/node.socket
...
nodestatus=`sudo systemctl status startRelayNode.service  | sed -n '3p' | xargs`
echo "Node Status: " $nodestatus >> $filepath/$outputfile

#notify by email
if [ $notifyByEmail = 1 ]
then
	mail -v -s "Maestro Health Report - $currdate" "$email" < $filepath/$outputfile
else
